...
const CommentController = {
  async create(req, res, next) {
    try {
      ...
      res.status(201).send(
        { message: "Se creó tu comentario!", comment }
      )
    } catch (error) {
      console.error(error);
  ...
  async update(req, res) {
    try {
      ...
      const comment = await Comment.findByIdAndUpdate(
        req.params._id,
        { ...req.body, userId: req.user._id, routeId: req.params._id },
        { new: true }
      );
      res.status(201).send(
        { message: `Comentario modificado con éxito`, comment }
      )
    } catch (error) {
      console.log(error);
      res.status(500).send(
  ...
  async getAll(req, res) {
    try {
      ...
      res.status(200).send(
        { Number_of_comments: comments.length, comments }
      )
    } catch (error) {
      console.log(error);
      res.status(500).send(
        { message: "No se pudieron conseguir los comentarios" }
      )
    }
  },
  ...
  async getAllPaginated(req, res) {
    try {
      ...
      const comments = await Comment.find().populate("userId")
        ...
        .skip((page - 1) * limit);
      res.status(200).send(
        { Number_of_comments: comments.length, comments, numberComments}
      )
    } catch (error) {
      console.log(error);
      res.status(500).send(
